...
const ingnoreBaseUrl = [/\/getValidation/,/\/checkoutValidation/,/\/registeredUser/,/\/session[\/]*\w*/,/\/user\/oauth[\/]*\w*/]
...
/*         console.log(req.baseUrl)
        console.log("@@@@@")
        console.log(ingnoreBaseUrl[i].test(req.baseUrl)) */
    ...
    if(bol) {
        ...
        // 查询数据库，如果方法在库里面就拦截，不在库里就不拦截
        console.log(req.method)
        if (req.method == 'GET'){
            const ary  = [/\/list/,/\/team/,/\/session[\/]*\w*/,/\/user\/oauth[\/]*\w*/]
            for (let i = 0 ;i < ary.length; i++){
                if( ary[i].test(req.baseUrl)){
                    next()
                }
            }
        }
