...
api.get('/cb', function(req, res) {
    ...
    request(opt).then((repos)=>{
        console.log(repos)
        console.log(repos.userInfo)
        console.log(repos.userInfo.truename)
        token = repos.userInfo.umtId
        name = repos.userInfo.truename
        cstnetId = repos.userInfo.cstnetId
        ...
        console.log(data)
        if (data.length === 0) {//
            return sessionModel.findOneAndUpdate({token},{$set:{last_action: new Date()}}, {rew: true})
        }
        console.log(data[2])
