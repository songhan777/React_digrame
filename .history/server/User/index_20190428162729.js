...
api.post('/logout', function (req, res) {
    ...
    req.session.last_action = new Date (1970,1,1)
    sessionModel.findOneAndUpdate({user_id:id},{$set: {last_action: new Date(1970,1,1)}},{new: true}).then((data) =>{ 
        res.send({offline: true})
    }).catch((err) =>{
        res.send({offlien: false})
    })
})
//校验 注册用户名是否重复
