...
class AddMember extends React.Component {
    ...
    componentDidMount() {
        ...
        if (JSON.stringify(data) === "{}") {
            recentlyMemberTeam({userId:111111}).then(data => {
                this.setState({recentlyList: data})
            })
        } else {
            recentlyMemberTeam({userId:111111}).then(newData => {
        ...
        if (JSON.stringify(this.props.data) === "{}") {
            ...
            if (teamValue !== "") {
                ...
                pushMember({
                    ...
                    if (data === 1) {
                        this.props.change(ary, teamValue, ary.length, addList.length, number, e, taskId)
                        this.props.shut()
                    } else {
                        alert("保存失败")
                    }
            ...
            if (this.state.teamValue !== "") {
                ...
                let newAry = {
                    ...
                    cooperationNumber: ary.length,//合作数量
    ...
    getSearchValue = (ev) => {
        this.setState({
            ...
            searchMemberTeam({text: this.state.searchValue,userId:111111}).then(data => {
                this.setState({
                    searchList: data
                })
            })
