...
class SetOneselfTeam extends React.Component {
    ...
    componentDidMount() {
        getTeamList({userId: 11111}).then((data) => {
            this.setState({
                wholeList: data,
                teamList: data.team,
    ...
    removeTeamList = (e, index, taskId) => {
        e.stopPropagation();
        console.log("执行删除");
        removeTeamList({userId: 111111, taskId: taskId}).then((data) => {
            if (data === 1) {
                let showIndex = index;
                console.log(index);
    ...
    removeMemberList = (e, index, taskId) => {
        e.stopPropagation();
        console.log("执行合作者删除");
        removeMemberList({userId: 111111, taskId: taskId}).then((data) => {
            if (data === 1) {
                let showIndex = index;
                this.setState({
                    ...
                    console.log(this.state.cooperatorList)
                })
            } else {
                alert("删除任务失败")
            }
                ...
                {value ?
                                        ...
                                        <CardActionArea className={classes.cardBox}>
                                            ...
                                            <Button className={classes.cardBoxIcon}>
                                                <Clear className={classes.iconDelete}
                                                       onClick={(e) => {
                                                           this.removeTeamList(e, index, item.taskId)
                                                       }}
                                                />
                                            </Button>
                                        </CardActionArea>
                    ...
                    <Card className={classes.partner}>
                        <div className={classes.cooperatorBox}>
                                    ...
                                    return (<Card key={index} className={classes.cooperationBox}
                                        ...
                                        <CardActionArea>
                                            <Button className={classes.cardBoxIcon}>
                                                <Clear className={classes.iconDelete}
                                                       onClick={(e) => {
                                                           this.removeMemberList(e, index, item.taskId)
                                                       }}
                                                />
                                            </Button>
                            ...
                            <div className={classes.cooperatorAdd} onClick={(e)=>{this.closeMember(e)}}>
                                <Fab color="primary" aria-label="Add" className={classes.fab}>
                                    <AddIcon/>
                                </Fab>
                            </div>
        ...
        return (
            <div className={classes.root}>
                ...
                {teamShow ? <AddTeam change={this.addTeam} shut={this.closeTeam} url={this.container}
                                     data={teamData} reset={this.resetData} handleList={this.handleTeamList} list={teamList}/> : null}
                {memberShow ?
                    <AddMember change={this.addCooperator} close={this.closeMember} url={this.container} reset={this.resetData}
                               number={cooperatorList.length} data={memberData} handleList={this.handleMemberList} list={cooperatorList} shut={this.shutMemberBox}/> : null}
                {show ? team : Member}
            </div>
        );
    }
